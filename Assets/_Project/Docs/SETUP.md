...
Once after cloning or when the team agrees:
...
3. In the editor menu run: **Tools → Fusion Multiplayer → Generate Scenes, Prefabs, And Build Settings**  
   ...
   - Prefabs: `PlayerData`, `PlayerAvatar`, `PlacedBlock` under `Assets/_Project/Prefabs/`
...
4. Open **Fusion → Network Project Config** (or the Fusion project settings window):
   - **Register network prefabs:** `PlayerData`, `PlayerAvatar`, `PlacedBlock`
