...
4. Fusion Hub: register **PlayerData**, **PlayerAvatar**, **PlacedBlock**; **Tick Rate** Client **32**, Send Index **1**; **Input Data Word Count** = **5** (mouse look); **Apply**
...
|------|--------|----------------|
| 1 | Press Play | `00_MainMenu` loads; yellow **FUSION MULTIPLAYER** title, readable buttons |
| 2 | Nickname + room → **Create / Host room** | Green status **Connecting to Photon room…**; buttons disabled briefly |
| 3 | Wait | Scene changes to lobby; **LOBBY** title; status **You are in the lobby.** |
| 4 | Lobby | Player list shows your nickname; **START GAME** visible (you are master in solo Play) |
| 5 | **START GAME** | `02_Game` loads; **SlotGrid** 5×2 at top; click slot → status changes immediately |
| 6 | Pick slot 0 | Status **Requesting slot 0…** then avatar spawns; mouse locks; **Slot N · WASD · Mouse** hint (~3s); panel hides |
| 7 | **Mouse** look | Cursor hidden; move mouse — yaw + pitch change; WASD moves relative to view |
| 8 | Click Game view | If cursor unlocked (alt-tab), **LMB** re-locks mouse look |
| 9 | **Enter** chat | Compose bar appears; cursor visible; type message |
| 10 | **Enter** again | Message sends; compose hides; mouse look returns |
| 11 | **E** / **Q** | **E** places cube on grid; **Q** removes crosshair block (not while chat open) |
| 12 | Master **End game** | Overlay; **Leave to main menu** returns to menu |
