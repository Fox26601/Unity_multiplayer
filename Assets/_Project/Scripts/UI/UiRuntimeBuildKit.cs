...
namespace FusionMultiplayer.UI
{
    ...
    internal static class UiRuntimeBuildKit
    {
        ...
        internal const string DropdownBackdropName = PmChannelBackdropName;
        ...
        {
            var listGo = new GameObject(listName, typeof(RectTransform), typeof(CanvasRenderer), typeof(RectMask2D));
        ...
        /// <summary>String-option dropdown for main menu form rows.</summary>
        internal static StringDropdownSelector CreateStringDropdownSelector(Transform row, float preferredHeight = 40f)
        {
            var root = new GameObject("StringDropdownSelector", typeof(RectTransform), typeof(CanvasRenderer),
                typeof(Image), typeof(StringDropdownSelector));
            root.transform.SetParent(row, false);
            var rt = root.GetComponent<RectTransform>();
            ConfigureDropdownFieldLayout(rt, preferredHeight, expandWidth: true);
            ...
            fieldButton.navigation = new Navigation { mode = Navigation.Mode.None };
            StyleDropdownFieldButton(fieldButton);

            var caption = CreateDropdownCaption(root.transform, "—");
            UiTypography.ApplyInputText(caption, false);
            caption.fontSize = 26;
            caption.color = UiTheme.TextPrimary;

            CreateDropdownArrow(root.transform);
            var (listRt, contentRt) = CreateDropdownPopupList(root.transform, "OptionList", 140f);

            var selector = root.GetComponent<StringDropdownSelector>();
            selector.BindRuntime(fieldButton, caption, listRt, contentRt, null, useInlinePopup: true);
            return selector;
        }

        private static void StyleDropdownFieldButton(Button button)
        {
